...
| 13 | Tail | Legacy wrapper consolidation (migrate ~49 sites → AppText/AppButton/AppCard, delete wrappers) | 04–11 | `2026-06-13-13-legacy-wrapper-consolidation.md` | ✅ |
...
| Date | Note |
|---|---|
| 2026-06-13 | **Plan 13 ✅ executed** (migration tail — user opted for full adoption): 4 commits (`25a80e15`→reordered into `eeb10394`+`9dcfe139`, plus tracker). 5 parallel per-file agents migrated ~49 call sites off the legacy text/button/card wrappers onto `AppText`/`AppButton`/`AppIconButton`/`AppCard`; controller deleted `Text.kt`/`Buttons.kt`/`Cards.kt` (all symbols grep-proven dead), modernized `NoContentView` onto `AppText`/`onSurfaceVariant`, extended the regression fence to 30 banned symbols. All-caps button styling eliminated; `customColors.secondaryTextColor`→`onSurfaceVariant` throughout text; touched off-grid dp tokenized. compile Android+iOS ✅, full test suite green, `installDebug` ✅. **Migration tail closed — zero legacy text/button/card wrapper duplication remains.** |
...
**✅ RESOLVED — Plan 13 executed (2026-06-13).** User chose full design-system adoption. All ~49 call sites migrated onto `AppText`/`AppButton`/`AppIconButton`/`AppCard` (5 parallel per-file agents + controller deletion); the 14 legacy wrapper symbols and their files (`Text.kt`/`Buttons.kt`/`Cards.kt`) deleted; all-caps `.uppercase()` button styling gone; `secondaryTextColor`→`onSurfaceVariant`; `NoContentView` modernized onto `AppText` (kept as the inline empty primitive, distinct from full-screen `EmptyState`). Regression fence extended to 30 symbols. compile both targets ✅, tests green, install ✅. Commits `eeb10394` (migrations) + `9dcfe139` (deletions/NoContentView/fence/doc).
