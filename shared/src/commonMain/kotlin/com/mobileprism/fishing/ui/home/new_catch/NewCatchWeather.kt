...
fun WeatherTypesSheet(onWeatherSelected: (SelectedWeather) -> Unit) {
    ...
    Column(
        ...
        weatherTypes.forEach {
            WeatherType(
                it,
                isOpened = openedList?.let { o -> o::class == it::class } ?: false,
                onWeatherSelected = onWeatherSelected
            ) {
                openedList =
                    if (openedList == null || openedList?.let { o -> o::class != it::class } != false) it else null
            }
        }
    }
}
...
fun <T> WeatherTypeTitle(
    ...
    Row(
        ...
        Text(text = stringResource(param.first().getNameRes))
...
fun <T> WeatherTypeItem(it: T, onWeatherSelected: (SelectedWeather) -> Unit) where T : StringOperation, T : WeatherIconPrefix {
        Row(modifier = Modifier
            ...
            .clickable { onWeatherSelected(SelectedWeather(it.stringRes, it.iconPrefix)) },
                verticalAlignment = Alignment.CenterVertically) {
            Text(modifier = Modifier.padding(4.dp), text = stringResource(it.stringRes))
        }
    }
...
fun NewCatchWeatherPrimary(
    ...
    if (weatherIconDialogState) {
        PickWeatherIconDialog(
            ...
            onWeatherSelected = {
                ...
                scope.launch {
                    onDescriptionChange(org.jetbrains.compose.resources.getString(it.stringRes))
                }
    ...
    OutlinedTextField(
        ...
        label = { Text(text = stringResource(Res.string.weather)) },
...
fun NewCatchTemperatureView(
    ...
    OutlinedTextField(
        ...
        isError = (temperature
            ...
            .apply { onError(this) },
        label = { Text(text = stringResource(Res.string.temperature)) },
        keyboardOptions = KeyboardOptions(
            imeAction = ImeAction.Next,
...
fun NewCatchPressureView(
    ...
    OutlinedTextField(
        ...
        label = { Text(text = stringResource(Res.string.pressure)) },
        keyboardOptions = KeyboardOptions(
            imeAction = ImeAction.Next,
            keyboardType = KeyboardType.Number
        ),
...
fun NewCatchWindView(
    ...
    OutlinedTextField(
        ...
        label = { Text(text = stringResource(Res.string.wind)) },
        keyboardOptions = KeyboardOptions(
            imeAction = ImeAction.Next,
            keyboardType = KeyboardType.Number
        ),
...
fun NewCatchMoonView(
    ...
    OutlinedTextField(
        ...
        onValueChange = { },
        trailingIcon = {
            Text(text = stringResource(Res.string.percent))
        },
        label = { Text(text = stringResource(Res.string.moon_phase)) },
        keyboardOptions = KeyboardOptions(
            imeAction = ImeAction.Next,
            keyboardType = KeyboardType.Number
        ),
